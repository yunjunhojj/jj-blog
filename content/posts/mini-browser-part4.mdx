...
이제 마지막 퍼즐 조각, **JavaScript 실행 환경**을 만들 차례입니다!

## JavaScript 인터프리터 구현

### 목표와 제약사항

완전한 JavaScript 파서를 만드는 것은 너무 복잡합니다. 대신:

- 브라우저의 네이티브 `Function` 생성자 활용
- DOM API, `console`, `setTimeout` 등을 주입
- 이벤트 루프와 비동기 처리에 집중

```typescript
// packages/core/runtime/scripts/interpreter.ts
import { EventLoop } from "../loop/eventLoop";

export class JSInterpreter {
  private globalEnv: Record<string, any> = {};
  private eventLoop: EventLoop;

  constructor(globalEnv: Record<string, any>, eventLoop: EventLoop) {
  ...
  run(code: string): any {
    try {
      // globalEnv의 키를 함수 매개변수로, 값을 인자로 전달
  ...
  updateGlobals(newGlobals: Record<string, any>): void {
    Object.assign(this.globalEnv, newGlobals);
  }

  // 비동기 API
  setTimeout(fn: Function, ms: number): number {
    return this.eventLoop.setTimeout(() => fn(), ms);
  }

  queueMicrotask(fn: Function): void {
...
```typescript
// packages/core/runtime/dom/bindings.ts
import { DOMNode } from "../../html/parser";
import { matchesSelector } from "../../style/matcher";
import { findNode } from "../dom/utils";
...
export function createDOMBindings(
  ...
  return {
    ...
    alert(message: any): void {
      ...
      // Canvas에 alert 박스 표시
      const alertNode: DOMNode = {
        type: "element",
        name: "div",
        attrs: { class: "alert-box" },
        children: [
          {
            type: "text",
            text: String(message),
...
export function matchesSelector(node: DOMNode, selector: string): boolean {
  ...
  if (selector.startsWith(".")) {
    ...
    return node.attrs?.class?.split(" ").includes(className) || false;
  }
...
async function main() {
  ...
  const reflow = () => {
    ...
    // 페인팅
    paint(currentLayoutTree, { ctx, pixelRatio });
  };
...
setInterval 실행 완료
setTimeout 실행 완료
setInterval 실행 완료 - 4번실행 (위에서 한번 실행되었기 때문에, 총 5번 실행됨)
setInterval 종료
```

## 이벤트 루프 작동 과정

위 코드의 실행 순서를 단계별로 분석해보겠습니다.
