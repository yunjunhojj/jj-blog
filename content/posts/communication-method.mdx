...
function LongPollingChat() {
  ...
  const isActiveRef = useRef(true); // 연결이 유지되는지 확인하는 참조
  ...
  useEffect(() => {
    const poll = async () => {
      ...
      try {
        // lastId 이후의 새 메시지를 요청 (서버가 대기)
        const res = await fetch(
          `/api/chat/long-poll?lastId=${lastIdRef.current}`
        );
        const data = await res.json();
...
function WebSocketChat() {
  ...
  const wsRef = useRef<WebSocket | null>(null); // WebSocket 인스턴스를 참조
...
/** 2. Polling */
let stats = { userCount: 10, orderCount: 3 };
setInterval(() => {
  stats.userCount += Math.floor(Math.random() * 5);
  stats.orderCount += Math.floor(Math.random() * 3);
}, 5000);
...
// 5초마다 모든 SSE 클라이언트에게 메시지 전송
setInterval(() => {
  const notification = {
    message: `서버 시각: ${new Date().toLocaleTimeString("ko-KR")}`,
    timestamp: Date.now(),
  };
...
전체 예제 코드와 실습 프로젝트는 [GitHub 저장소에서 확인할 수 있습니다.  
https://github.com/yunjunhojj/communication-method](https://github.com/yunjunhojj/communication-method)
